...
package org.jboss.errai9.databinding.client.components;
...
//import org.jboss.errai9.common.client.dom.HTMLElement;
import org.jboss.errai9.common.client.api.elemental2.IsElement;
...
/**
 * A component that binds a list of Bindable models to a displayed list of UI components. Supports selection of
...
public interface ListComponent<M, C extends TakesValue<M>> extends IsElement, TakesValue<List<M>>, BindableListChangeHandler<M> {
  ...
  static <M, C extends TakesValue<M> & IsWidget> Builder<M, C> forIsWidgetComponent(final Supplier<C> supplier,
          final Consumer<C> destroyer) {
    return new Builder<>(root -> new DefaultListComponent<>(root, supplier, destroyer,
            c -> {
                HTMLElement e = Js.uncheckedCast(c.asWidget().getElement());
                return e;
            }));
  }
  ...
  static class Builder<M, C extends TakesValue<M>> {
    ...
    public ListComponent<M, C> inElement(final String tagName) {
        HTMLElement e = Js.uncheckedCast(Document.get().createElement(tagName));
