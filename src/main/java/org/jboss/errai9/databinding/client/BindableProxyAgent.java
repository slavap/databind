...
import org.jboss.errai9.common.client.api.Assert;
//import org.jboss.errai9.common.client.api.IsElement;
import org.jboss.errai9.common.client.ui.ElementWrapperWidget;
...
import org.jboss.errai9.databinding.client.api.StateSync;
import org.jboss.errai9.databinding.client.api.handler.list.BindableListChangeHandler;
...
/**
 * Manages bindings and acts in behalf of a proxy to keep the target model and bound widgets in sync.
 * <p>
 * An agent will:
 * <ul>
...
public class BindableProxyAgent<T> implements HasPropertyChangeHandlers {
  ...
  protected final Map<String, PropertyType> propertyTypes = new HashMap<>();
  ...
  protected BindableProxyAgent(final T proxy) {
    this.proxy = proxy;
  }
  ...
  private static boolean mustBeIgnored(String property) {
      if ("$H".equals(property)) return true;
      if (property == null || property.isEmpty()) return false; // backward compatibility, probably never happens
      if (property.endsWith(".$H")) return true; // array element
      return false;
  }
  ...
  protected void copyValues() {
    for (final String property : propertyTypes.keySet()) {
      if (!mustBeIgnored(property)) {
        knownValues.put(property, Bindable.support.get(proxy, property));
      }
    }
  }
  ...
  private static Optional<Supplier<Object>> maybeCreateUIGetter(final Object component) {
    ...
    else if (component instanceof org.jboss.errai9.common.client.api.elemental2.IsElement) {
      return maybeCreateElementValueGetter(BoundUtil.asElement(((org.jboss.errai9.common.client.api.elemental2.IsElement) component).getElement()));
    }
  ...
  private static Consumer<Object> modelUpdater(final Object component, final Converter converter, final String lastSubProperty,
          final BindableProxyAgent<?> agent) {
    return uiValue -> {
      final Object oldValue = Bindable.support.get(agent.proxy, lastSubProperty);
  ...
  private static Optional<Supplier<Object>> createTakesValueGetter(final TakesValue component) {
    return Optional.ofNullable(() -> component.getValue());
  }

  private static Optional<Supplier<Object>> createHasTextGetter(final HasText component) {
    return Optional.ofNullable(() -> component.getText());
  }

  private static Optional<Supplier<Object>> maybeCreateElementValueGetter(final Element element) {
  ...
  private Binding bindNestedProperty(final Object component, final String property, final Converter<?, ?> converter,
      ...
      final BindableProxyAgent<?> nestedAgent = Bindable.support.getBindableProxyAgent(nestedBinder.getModel());
  ...
  private void trySettingModelProperty(final String property, final Converter converter, final Object uiValue,
    ...
    try {
      Bindable.support.set(proxy, property, newValue, false/*fireUpdateEvent*/);
  ...
  private static boolean isCausedByNullOrUndefined(final Throwable t) {
   ...
   * <br> So it means that ANY field is literally possible, so NonExistingPropertyException makes no sense.
   */
  private static PropertyType getPropertyType(final Object proxy, final String property) {
    final BindableProxyAgent<?> agent = Bindable.support.getBindableProxyAgent(proxy);
    if (property.contains(".")) {
      ...
      Object topLevelProxy = null;
      if (topLevelType instanceof MapPropertyType) {
        //topLevelProxy = DataBinder.forMap(((MapPropertyType) topLevelType).getPropertyTypes()).getModel();
      }
      else {
        //topLevelProxy = BindableProxyFactory.getBindableProxy(topLevelType.getType().getName());
        topLevelProxy = Bindable.support.get(proxy, topLevelProperty);
        if (topLevelProxy != null) {
            topLevelProxy = Bindable.support.observe(topLevelProxy); // to make sure agent is created for this proxy
      ...
      PropertyType pt = Bindable.support.getBeanProperty(proxy, property);
  ...
  private PropertyType resolvePropertyInRuntime(String property) {
      PropertyType pt = propertyTypes.get(property);
      if (pt == null) {
          pt = Bindable.support.getBeanProperty(proxy, property); // attempt to resolve field in runtime (js allows fields adding on the fly)
          if (pt != null) {
              propertyTypes.put(property, pt);
              knownValues.put(property, Bindable.support.get(proxy, property));
          }
      }
      return pt;
  ...
  private Binding createBinding(final Object component, final String property, final Converter converter,
    ...
    PropertyType pt = resolvePropertyInRuntime(property);

    if (pt != null && pt.isBindable() && pt.isList()) {
        mayBeArrayChangeHandler(property, event -> {
            updateWidgetsAndFireEvent(true/*sync*/, property, event.getOldValue(), event.getNewValue());
        });
...
//      }
//      else {
//        Bindable.support.set(proxy, property, ensureBoundListIsProxied(property));
//      }
    }
  ...
  private Converter findConverter(final String property, final Class<?> propertyType, final Object component, final Converter userProvidedConverter) {
    ...
    if (userProvidedConverter != null) {
      if (!Object.class.equals(propertyType)) { // for native models we usually don't have enough type information, so consider Object compatible with provided converter
          validateTypes(property, propertyType, userProvidedConverter.getModelType(), "model");
      }
      componentValueType.ifPresent(actualType -> {
          Class converterType = userProvidedConverter.getComponentType();
          if (JavaScriptObject.class.equals(actualType) && Object.class.equals(converterType)) {
              // widget can be bound to JavaScriptObject in case of native models
          } else if (JavaScriptObject.class.equals(converterType) && actualType.getComponentType() != null
                  && actualType.getComponentType().equals(converterType)) {
              // could be JQMList to js Array binding
          } else {
              validateTypes(property, actualType, converterType, "widget");
          }
      });
      ...
      if (converter == null) {
        throw new RuntimeException(
                "Cannot convert between " + propertyType.getName() + " and " + effectiveComponentType.getName()
                        ...
                        + Bindable.support.unwrap(proxy).getClass().getName());
      }
 ...
 * @param property
 */
private static void validateTypes(final String property, final Class<?> actualType, final Class<?> converterType, final String modelOrWidget) {
    if (Object.class.equals(converterType)
            && (String.class.equals(actualType) || Boolean.class.equals(actualType) || Double.class.equals(actualType))) {
      // in native databinding Object is default for field type, because real type is "unknown"
      return;
    }
  ...
  private static boolean oneTypeIsInterface(final Class<?> propertyType, final Class<?> converterModelType) {
  ...
  private static Supplier<Map<Class<? extends GwtEvent>, HandlerRegistration>> addHandlers(
    ...
    /*else if (component instanceof IsElement) {
      ...
      registrar = mergeNativeChangeEventListener(((org.jboss.errai9.common.client.api.elemental2.IsElement) component).getElement(), uiGetter, modelUpdater, registrar);
    }
    else if (isElement(component)) {
    ...
    if (bindOnKeyUp) {
      ...
      /*else if (component instanceof IsElement) {
        ...
        registrar = mergeNativeKeyUpEventListener(((org.jboss.errai9.common.client.api.elemental2.IsElement) component).getElement(), uiGetter, modelUpdater, registrar);
      }
      else if (isElement(component)) {
  ...
  private DataBinder createNestedBinder(final String property, final StateSync initialState) {
    ...
    PropertyType pt = resolvePropertyInRuntime(bindableProperty);
    ...
    if (binder == null) {
      if (Bindable.support.get(proxy, bindableProperty) == null) {
        binder = DataBinder.forType(propertyTypes.get(bindableProperty).getType());
      }
      else {
        binder = DataBinder.forModel(Bindable.support.get(proxy, bindableProperty));
      }
    ...
    else if (Bindable.support.get(proxy, bindableProperty) != null) {
      binder.setModel(Bindable.support.get(proxy, bindableProperty), initialState, true);
    }
    Bindable.support.set(proxy, bindableProperty, binder.getModel());
    knownValues.put(bindableProperty, binder.getModel());

    if (property.indexOf('.') != property.lastIndexOf('.')) {
      Bindable.support.getBindableProxyAgent(binder.getModel()).createNestedBinder(
              property.substring(property.indexOf('.')+1), initialState);
    }

    return binder;
  }

  private static void validatePropertyExpr(final String property) {
  ...
  public void unbind(final Binding binding) {
    ...
    if (dotPos > 0) {
      ...
      if (binder != null) {
        final BindableProxyAgent<T> nestedAgent = Bindable.support.getBindableProxyAgent(binder.getModel());
    ...
    if (bindings.isEmpty()) {
      Bindable.support.agentUnbound(this);
    }
  }
  ...
  protected void updateWidgetsAndFireEvents() {
    for (final String property : propertyTypes.keySet()) {
      ...
      final Object actualValue = Bindable.support.get(proxy, property);
      ...
      if ((knownValue == null && actualValue != null) || (knownValue != null && !knownValue.equals(actualValue))) {
        ...
        if (nestedBinder != null) {
          ...
          Bindable.support.set(proxy, property, nestedBinder.getModel());
        }
   ...
   */
  protected <P> void updateWidgetsAndFireEvent(final boolean sync, final String property, final P oldValue, final P newValue) {
  ...
  private <P> void updateWidgetsAndFireEvent(final boolean sync, final String property, final P oldValue, final P newValue,
    ...
    Collection<Binding> propBindings = bindings.get(property);
    for (final Binding binding : propBindings) {
      ...
      if (component instanceof TakesValue) {
        ...
        updateComponentValue(newValue, (HasText) component, converter);
              ...
              ||*/ component instanceof org.jboss.errai9.common.client.api.elemental2.IsElement || isElement(component)) {
  ...
  private static Object getUIPart(final Object component) {
    /*if (component instanceof IsElement) {
      return ((IsElement) component).getElement();
    } else*/ if (component instanceof org.jboss.errai9.common.client.api.elemental2.IsElement) {
      return ((org.jboss.errai9.common.client.api.elemental2.IsElement) component).getElement();
    } else {
      return component;
    }
  }

  private static <P> void updateComponentValue(final P newValue, final HasText component, final Converter converter) {
  ...
  private static <P> void updateComponentValue(final P newValue, final TakesValue component, final Converter converter) {
    final Object widgetValue = converter.toWidgetValue(newValue);
  ...
  private <P> boolean maybeFirePropertyChangeEvent(final String property, final P oldValue, final P newValue) {
    ...
    if (!mustBeIgnored(property) || propertyTypes.size() == 1) {
  ...
  private void syncState(final Object component, final String property, final Converter converter,
    ...
    if (initialState != null) {
      final Object modelValue = Bindable.support.get(proxy, property);
      ...
      if (initialState == StateSync.FROM_MODEL) {
        ...
        Bindable.support.set(proxy, property, newValue);
...
//    final List oldList = (List) Bindable.support.get(proxy, property);
//    final List newList = ensureBoundListIsProxied(property, oldList);
//    if (oldList != newList)
//      updateWidgetsAndFireEvent(true, property, Bindable.support.get(proxy, property), newList);
//
//    return newList;
//  }
...
//  }
  ...
  public <P> PropertyChangeUnsubscribeHandle addPropertyChangeHandler(final String property, final PropertyChangeHandler<P> handler) {
    ...
    if (dotPos > 0) {
      ...
      unsubHandles.add(new PropertyChangeUnsubscribeHandle() {
        ...
        PropertyChangeUnsubscribeHandle h = mayBeArrayChangeHandler(property, (PropertyChangeHandler<Object>) handler);
        if (h != null) unsubHandles.add(h);
    }
  ...
  private PropertyChangeUnsubscribeHandle mayBeArrayChangeHandler(String property, PropertyChangeHandler<Object> handler) {
      PropertyType t = getPropertyType(proxy, property);
      if (t != null && t.isBindable() && t.isList() && t.getType() == JavaScriptObject.class) { // bindable js Array
          final DataBinder nested = createNestedBinder(property + ".*", StateSync.FROM_MODEL);
          return nested.addPropertyChangeHandler("*", event -> {
              // here we will get ALL change notifications for array's elements.
              // they could be VERY long and contain cycles, so better to restrict them.
              // example: 0.ctgs.0.helper_47_g$.presenter_11_g$.selGenItems.0.ctgs.$H
              String s = event.getPropertyName();
              if (mustBeIgnored(s)) return;
              int dots = 0;
              ...
              Object oldV = Js.undefined(); // we don't know old value, though can construct it based on event
              Object newV = Bindable.support.get(proxy, property);
              PropertyChangeEvent<Object> ev = new PropertyChangeEvent<>(proxy, property, oldV, newV);
              if (handler != null) handler.onPropertyChange(ev);
              else propertyChangeHandlerSupport.notifyHandlers(ev);
          });
      }
      return null;
  }
    ...
    for (final String property : propertyTypes.keySet()) {
      ...
      if ((curValue == null && oldValue != null) || (curValue != null && !curValue.equals(oldValue))) {
        ...
        if (nestedBinder != null && otherNestedBinder != null) {
          final BindableProxyAgent nestedAgent = Bindable.support.getBindableProxyAgent(nestedBinder.getModel());
          final BindableProxyAgent otherNestedAgent = Bindable.support.getBindableProxyAgent(otherNestedBinder.getModel());
  ...
  public void clearModelHandlers() {
    ...
    for (final DataBinder<?> binder : binders.values()) {
      BindableProxyAgent<?> a = getAgent(binder);
      if (a != this) a.clearModelHandlers();
    }
  }

  private static BindableProxyAgent<?> getAgent(final DataBinder<?> binder) {
    return Bindable.support.getBindableProxyAgent(binder.getModel());
  }

  protected void updateBindersProxy(T newProxy) {
      for (final DataBinder<?> binder : binders.values()) {
          DataBinder<T> bi = (DataBinder<T>) binder;
          bi.updateProxy(newProxy);
      }
  }
}
